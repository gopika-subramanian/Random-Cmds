...
- [ ] SSL/TLS Vulnerabilities + Weak Ciphers
- [ ] Missing Security Headers: Content-Security-Policy, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, X-XSS-Protection, HTTP Strict Transport Security, Cache-Control
...
- [ ] Content Spoofing: Free Text Injection (Send false links with trusted site information)
- [ ] Broken Access Control: Resource sharing to all users aka premium members links
- [ ] Lack of Input Validation (Check random values to 200 OK params)
- [ ] Clickjacking 
- [ ] Hardcoded Credentials
- [ ] Process improvement (User verification not found)
- [ ] Lack of Account Lockout (Does the application lockout user account after more than 3-10 invalid attempts?)
- [ ] Weak Password Policy (Can you set a weak password?)
- [ ] Session Token in URL (Check token values in URL)
- [ ] Lack of Rate-Limit on APIs - Lack of Faulty 
- [ ] Username Enumeration from all sources (Registration, Login, Forgot Password etc ---> Invalid values shown in response)
- [ ] Lack of Session Timeout
- [ ] Formula Injection (CSV, sheets expoting: 1+1 OR =hyperlink(“Link”,”Click”)
- [ ] Faulty Logout (Does the cookie/auth token actually expire?, No logout option in application)
- [ ] Information Leakage - Directory Listing Enabled, Stack Trace, Internal IP Disclosure, etc.
- [ ] Cross-Origin Resource Sharing - Working PoC
- [ ] Host Header Injection - Forgot password, registration features
- [ ] Weak Password Reset Implementation
- [ ] Captcha Bypass
- [ ] Token Leakage via Referer
- [ ] Lack of Password Confirmation (Change Email / password)
